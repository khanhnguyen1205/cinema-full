...
import { getMovies, getCinemas, getCities, getAllShowtimes } from "services/api";
...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([getMovies(), getCinemas(), getCities(), getAllShowtimes()])
      .then(([m, c, ct, st]) => {
        setMovies(m);
        setCinemas(c);
        setCities(ct);
  ...
  const prev = () => setHeroIndex(i => (i - 1 + featured.length) % featured.length);
    ...
    <div className="page home-page">
      ...
      >
        ...
        <div
          ...
          style={active.poster ? {
            backgroundImage:
              `linear-gradient(to left, rgba(0,0,0,0) 0%, rgba(10,10,10,0.6) 40%, rgba(10,10,10,1) 80%), url(${active.poster})`,
        ...
        <div className="hero-content" key={active.id}>
          ...
          <div className="hero-actions">
            <button className="btn-primary" onClick={() => navigate(`/movie/${active.id}`)}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="white" style={{ marginRight: 8 }}>
                <polygon points="5 3 19 12 5 21 5 3" />
              </svg>
              Đặt vé
            </button>
            <button className="btn-secondary" onClick={() => navigate(`/movie/${active.id}`)}>Chi tiết</button>
          </div>
        ...
        <button className="hero-arrow hero-arrow-prev" aria-label="Phim trước" onClick={prev}>‹</button>
        <button className="hero-arrow hero-arrow-next" aria-label="Phim sau" onClick={next}>›</button>
      </section>
      ...
      <section className="genres-section">
        ...
        <div className="genre-grid">
          {genreStats.map(({ genre, count }) => (
            ...
            >
              <span className="genre-tile-bar" style={{ background: GENRE_COLORS[genre] || GENRE_COLORS.Default }} />
      ...
      <section className="cinemas-section">
        <div className="stats-strip">
          <div className="stat"><span className="stat-num">{movies.length}</span><span className="stat-label">Phim</span></div>
          <div className="stat"><span className="stat-num">{cinemas.length}</span><span className="stat-label">Rạp chiếu</span></div>
          <div className="stat"><span className="stat-num">{cities.length}</span><span className="stat-label">Thành phố</span></div>
          <div className="stat"><span className="stat-num">{showtimeCount}</span><span className="stat-label">Suất chiếu</span></div>
        </div>
        ...
        <div className="section-header cinemas-header">
          ...
          <button className="view-all" onClick={() => navigate("/cinemas")}>Tất cả rạp →</button>
        </div>

        <div className="cinema-grid">
          {cinemas.map(c => (
            <button key={c.id} className="cinema-card" onClick={() => navigate(`/cinema/${c.id}`)}>
              <span className="cinema-card-mark" />
              <span className="cinema-card-body">
                <span className="cinema-card-name">{c.name}</span>
                <span className="cinema-card-city">{cityName[c.cityId] || "—"}</span>
              </span>
        ...
        <div className="home-cta">
          <div className="home-cta-content">
            <h2 className="home-cta-title">Sẵn sàng cho <span className="red">suất chiếu</span> tiếp theo?</h2>
            <p className="home-cta-sub">Chọn phim, chọn ghế và đặt vé chỉ trong vài bước.</p>
          </div>
          <button className="btn-primary home-cta-btn" onClick={() => navigate("/movies")}>Đặt vé ngay</button>
        </div>
      </section>
