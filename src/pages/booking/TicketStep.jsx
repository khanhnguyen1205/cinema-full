...
const fmtDate = (iso) => iso ? new Date(iso).toLocaleDateString("vi-VN", { weekday: "short", day: "2-digit", month: "short", year: "numeric" }) : "—";
  ...
  return (
    <div className="ticket-step">
      <div className="ts-success">
        ...
        <p className="ts-success-sub">Vé điện tử của bạn đã sẵn sàng. Xuất trình mã QR tại rạp để vào cửa.</p>
      </div>
      ...
      <div className="eticket">
        <div className="eticket-main">
          ...
          <h3 className="eticket-title">{movie?.title || `Phim #${booking.movieId}`}</h3>
          <p className="eticket-cinema">{cinema?.name}{room ? ` · ${room.name} · ${room.type}` : ""}</p>

          <div className="eticket-grid">
            <div><span className="eticket-label">Ngày</span><span className="eticket-value">{fmtDate(showtime?.time)}</span></div>
            <div><span className="eticket-label">Giờ</span><span className="eticket-value">{fmtTime(showtime?.time)}</span></div>
            <div><span className="eticket-label">Ghế</span><span className="eticket-value eticket-seats">{(booking.seats || []).join(", ") || "—"}</span></div>
            <div><span className="eticket-label">Thanh toán</span><span className="eticket-value">{METHOD_LABEL[booking.paymentMethod] || booking.paymentMethod}</span></div>
          </div>

          {booking.concessions?.length > 0 && (
            <div className="eticket-fnb">
              <span className="eticket-label">Bắp nước</span>
              <span className="eticket-value">{booking.concessions.map((c) => `${c.name} ×${c.qty}`).join(", ")}</span>
            </div>
          )}

          <div className="eticket-total">
            <span>Tổng cộng</span>
            <span className="eticket-total-amount">{fmt(booking.totalPrice)}</span>
          </div>
        </div>
