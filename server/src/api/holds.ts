...
// Bỏ mọi ghế user đang giữ khỏi map của suất (dùng cho cả nhả lẫn đặt lại).
function dropHoldsOf(seatMap: Map<string, Hold>, userId: number): void {
  for (const [seat, h] of seatMap)
    if (h.userId === userId) seatMap.delete(seat);
}
...
export function releaseHolds(
  ...
  dropHoldsOf(seatHolds(showtimeId), userId);
}
