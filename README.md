...
## Testing
- **496 Vitest tests** in two projects — `client` (happy-dom + Testing Library) and `server` (supertest against the real Express app with a mocked Prisma, so it runs without a database). Coverage: **91.0% statements**, enforced in CI by thresholds in `vite.config.mjs` (`npm run test:cov`).
